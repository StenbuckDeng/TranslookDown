...
<div class="container">
  ...
  <div class="header-row">
    ...
    <div class="header-right">
      ...
      <button class="btn-close" onclick="closeApp()" title="Close">&#10005;</button>
    </div>
  </div>
  ...
  <div class="main-grid">
    ...
    <div class="main-panel">
      ...
      <div class="input-area">
        <div class="input-row">
          <input type="text" id="urlInput" placeholder="Paste video link https://..." autocomplete="off" spellcheck="false">
          <button class="btn-paste" id="pasteBtn" onclick="pasteAndDownload()" title="Paste &amp; Download">&#128203; Paste</button>
          <button class="btn-download" id="downloadBtn" onclick="startDownload()">
            <span id="btnText">&#11015;&#65039; Download</span>
          </button>
        </div>
      </div>
      ...
      <div class="panel-tabs">
        <button class="panel-tab active" id="tab_downloads" onclick="switchTab('downloads')">&#11015;&#65039; Downloads (<span id="downloadCount">0</span>)</button>
        <button class="panel-tab" id="tab_history" onclick="switchTab('history')">&#128203; History (<span id="historyCount">0</span>)</button>
      </div>
      ...
      <div class="panel-content">
        <div id="contentDownloads">
          <div id="downloadList">
            <div class="empty-state">
              <div class="emoji">&#127744;</div>
              <div>No downloads yet. Paste a link to get started~</div>
            </div>
          </div>
        </div>
        <div id="contentHistory" style="display:none;">
          ...
          <button class="btn-clear-all" id="clearAllBtn" style="display:none" onclick="clearHistory()">&#128465;&#65039; Clear</button>
        </div>
      </div>
    </div>
    ...
    <div class="options-panel">
      ...
      <div class="option-group">
        <div class="toggle-row">
          <span class="toggle-row-label">Subtitles</span>
      ...
      <div class="option-group">
        <div class="toggle-row">
          <span class="toggle-row-label">Thumbnail</span>
          <label class="toggle-switch">
      ...
      <div class="option-group">
        <div class="toggle-row">
          <span class="toggle-row-label">Metadata</span>
          <label class="toggle-switch">
  ...
  <div class="faq-section">
    ...
    <div class="faq-item" onclick="toggleFaq(this)">
      <button class="faq-question">Which sites are supported? <span class="faq-arrow">&#9660;</span></button>
      <div class="faq-answer">YouTube, X (Twitter), LinkedIn, Bilibili, Douyin, Instagram, TikTok and thousands more.</div>
    </div>
    <div class="faq-item" onclick="toggleFaq(this)">
      <button class="faq-question">Where are videos saved? <span class="faq-arrow">&#9660;</span></button>
      <div class="faq-answer">In the 'downloads' folder next to the program. Click the 'Folder' button to open it.</div>
    </div>
    <div class="faq-item" onclick="toggleFaq(this)">
      <button class="faq-question">What if download fails? <span class="faq-arrow">&#9660;</span></button>
      <div class="faq-answer">Some sites may have regional restrictions. Try updating yt-dlp to the latest version.</div>
    </div>
    <div class="faq-item" onclick="toggleFaq(this)">
      <button class="faq-question">Can I access from phone? <span class="faq-arrow">&#9660;</span></button>
      <div class="faq-answer">Yes! Make sure your phone and PC are on the same network. Click the QR button to scan.</div>
    </div>
  </div>
</div>
...
<div class="qr-popup" id="qrPopup" style="display:none">
  <div class="qr-popup-content">
    <div class="qr-popup-title">&#128241; Scan QR to access</div>
...
async function pasteAndDownload() {
  try {
    ...
    if (text && text.trim().startsWith('http')) {
      ...
      showToast('Pasted and downloading!', 'success');
      startDownload();
    } else {
      document.getElementById('urlInput').value = text;
      showToast('Pasted, but content is not a valid URL', 'error');
    }
  } catch(e) {
    showToast('Cannot read clipboard, please paste manually', 'error');
  }
}
...
async function startDownload() {
  ...
  btn.disabled = true;
  btnText.innerHTML = '<span class="spinner"></span> Requesting...';
  isDownloading = true;
...
function renderDownloads() {
  ...
  if (keys.length === 0) {
    list.innerHTML = '<div class="empty-state"><div class="emoji">&#127744;</div><div>No downloads yet. Paste a link to get started~</div></div>';
    return;
  }
  var html = '';
  for (var i = 0; i < keys.length; i++) {
    ...
    var statusLabel = d.status === 'starting' ? 'Waiting' : d.status === 'downloading' ? 'Downloading' : d.status === 'completed' ? 'Completed' : 'Failed';
...
function renderHistory() {
  ...
  if (historyData.length === 0) {
    list.innerHTML = '<div class="empty-state"><div class="emoji">&#128214;</div><div>No history yet~</div></div>';
  ...
  for (var i = 0; i < historyData.length; i++) {
    var h = historyData[i];
    var statusLabel = h.status === 'completed' ? 'Completed' : 'Failed';
    ...
    if (h.filepath && h.filepath !== 'unknown' && h.filepath !== 'already_downloaded') {
      html += '<button class="btn-sm open" onclick="openFile(\'' + escapeJs(h.filepath) + '\')">&#128194; Open</button>';
    }
    html += '<button class="btn-sm copy" onclick="copyUrl(\'' + escapeJs(h.url) + '\')">&#128203; Copy URL</button>';
...
def main():
    ...
    print(f"  History file: {get_history_path()}")
    print("=" * 50)
    print("  Starting...")
